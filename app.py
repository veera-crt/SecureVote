...
def index():
    ...
    try:
        ...
        election_data = None
        if election:
            # Get top 2 candidates for this election
            cur.execute("""
            ...
            # We need total votes for the ELECTION to calculate accurate percentage, not just top 2
            cur.execute("SELECT SUM(vote_count) FROM candidates WHERE election_id = %s", (election[0],))
            election_total_votes = cur.fetchone()[0] or 0
...
def admin_dashboard():
    ...
    try:
        ...
        # Active Elections
        cur.execute("SELECT COUNT(*) FROM elections WHERE status = 'active'")
        active_elections = cur.fetchone()[0]
